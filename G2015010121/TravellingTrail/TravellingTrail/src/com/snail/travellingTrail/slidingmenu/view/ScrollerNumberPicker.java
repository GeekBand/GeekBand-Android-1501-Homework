...
public class ScrollerNumberPicker extends View {
		...
		for (ItemObject item : itemList) {
			if (item.isSelected()) {
				if (onSelectListener != null)
					onSelectListener.endSelect(ScrollerNumberPicker.this.getId(),item.id, item.itemText);
				break;
			}
		}
	}
		...
		if (move > 0) {
			for (int i = 0; i < itemList.size(); i++) {
				if (itemList.get(i).isSelected()) {
					...
					if (onSelectListener != null)
						onSelectListener.endSelect(ScrollerNumberPicker.this.getId(),itemList.get(i).id,
	...
	private void onSelectListener() {
		...
		for (ItemObject item : itemList) {
			if (item.isSelected()) {
				onSelectListener.selecting(ScrollerNumberPicker.this.getId(),item.id, item.itemText);
			}
		}
	}
